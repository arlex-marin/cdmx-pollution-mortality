...
from .utils import POLLUTANTS, format_pvalue
...
def plot_temporal_trends(df_pm25):
    ...
    yearly_avg = (
        both_sex.groupby("year")
        .agg(
            {
                "pm25": "mean",
                "pm10": "mean",
                "age_standardized_rate": "mean",
                "crude_rate": "mean",
            }
        )
        .reset_index()
    ...
    axes[0].set_title(
        "Air Pollution Trends in Mexico City (2004-2022, 14 Alcaldías)",
        fontsize=12,
        fontweight="bold",
    )
    ...
    axes[1].plot(
        yearly_avg["year"],
        yearly_avg["age_standardized_rate"],
        "s-",
    ...
    axes[1].set_title(
        "Lung Cancer Mortality Trends in Mexico City (2004-2022, 14 Alcaldías)",
        fontsize=12,
        fontweight="bold",
    )
...
def plot_correlation_scatter(df_pm25):
    ...
    scatter = ax.scatter(
        both_sex["pm25"],
        both_sex["age_standardized_rate"],
        c=both_sex["year"],
        cmap="viridis",
        alpha=0.6,
        s=50,
    ...
    ax.set_title(
        "Association between PM2.5 and Lung Cancer Mortality\nMexico City, 2004-2022 (14 Alcaldías)",
        fontsize=12,
        fontweight="bold",
    )
...
def plot_alcaldia_boxplot(df_pm25):
    ...
    sns.boxplot(
        data=both_sex,
        x="alcaldia",
        y="age_standardized_rate",
        order=alcaldia_order,
        palette="Reds",
        ax=ax,
    )
    ax.set_xlabel("")
    ax.set_ylabel("Age-Standardized Mortality Rate (per 100,000)", fontsize=11)
    ax.set_title(
        "Distribution of Lung Cancer Mortality Rates by Alcaldía\nMexico City, 2004-2022 (14 Alcaldías)",
        fontsize=12,
        fontweight="bold",
    )
...
def plot_pm25_by_alcaldia(df_pm25):
    ...
    ax.set_title(
        "Distribution of PM2.5 Concentrations by Alcaldía\nMexico City, 2004-2022 (14 Alcaldías)",
        fontsize=12,
        fontweight="bold",
    )
    ax.tick_params(axis="x", rotation=45)
...
def plot_regression_coefficients(models):
    ...
    ax.set_title(
        "Effect of PM2.5 on Age-Standardized Lung Cancer Mortality\n(2004-2022, 14 Alcaldías)",
        fontsize=12,
        fontweight="bold",
    )
    ax.grid(True, alpha=0.3)
...
def plot_sex_specific_effects(sex_models):
    ...
    sexes = ["Male", "Female"]
    coefs = [
        sex_models["Male"].params["pm25_10"],
        sex_models["Female"].params["pm25_10"],
    ]
    cis = [
        [
    ...
    ax.set_ylabel("Coefficient for PM2.5 (per 10 μg/m³)")
    ax.set_title(
        "Sex-Specific Effects of PM2.5 on Lung Cancer Mortality\n(2004-2022, 14 Alcaldías)",
        fontsize=12,
        fontweight="bold",
    )
...
def plot_correlation_heatmap(df_pm25):
    ...
    corr_cols = [p for p in POLLUTANTS if p in both_sex.columns] + [
        "crude_rate",
    ...
    ax.set_title(
        "Correlation Matrix: Air Pollutants and Mortality Rates\n(2004-2022, 14 Alcaldías)",
        fontsize=14,
        fontweight="bold",
    )
...
def create_all_visualizations(
    df_pm25: pd.DataFrame, models: dict | None = None, sex_models: dict | None = None
) -> None:
